'use client'
import { coachSignUpSchema } from "@/lib/validations/auth"
...
export default function CoachSignUpForm()
{
    ...
    const [stage, setStage] = useState<'username' | 'email' | 'otp' | 'game'>('username')
    ...
    const form = useForm<z.infer<typeof coachSignUpSchema>>({
    ...
    async function onSubmit(values: z.infer<typeof coachSignUpSchema>) {
        setLoading(true)
        try
        {
            const signInResult = await signIn('credentials', { email: values.email, username: values.username, playName: values.playName, role: 'coach', otp: otp, redirect: false })
    ...
    return (
        <div className='flex flex-col items-center justify-start gap-16 flex-1 w-full max-md:h-screen max-md:w-screen'>
            <div className='flex items-center justify-center md:justify-between gap-4 w-full max-md:flex-wrap'>
                ...
                <div className={cn('flex items-center justify-center min-w-6 min-h-6 md:min-w-8 md:min-h-8 rounded-full text-black max-md:text-base', ['otp', 'game'].includes(stage) ? 'bg-[#1D8841]' : 'bg-white', stage !== 'email' && 'max-md:hidden')}>
                    {['otp', 'game'].includes(stage) ? <Check stroke='#fff' size={24} /> : '2'}
                </div>
                <p className={cn('text-base md:text-lg font-bold', ['otp', 'game'].includes(stage) ? 'text-[#8D8D99]' : 'text-white', stage !== 'email' && 'max-md:hidden')}>Email</p>
                <ChevronRight size={24} stroke='#C66060' className={'max-md:hidden'} />
                <div className={cn('flex items-center justify-center min-w-6 min-h-6 md:min-w-8 md:min-h-8 rounded-full text-black max-md:text-base', ['otp'].includes(stage) ? 'bg-[#1D8841]' : 'bg-white', stage !== 'game' && 'max-md:hidden')}>
                    {['otp'].includes(stage) ? <Check stroke='#fff' size={24} /> : '3'}
                </div>
                <p className={cn('text-base md:text-lg font-bold', ['otp'].includes(stage) ? 'text-[#8D8D99]' : 'text-white', stage !== 'game' && 'max-md:hidden')}>Game</p>
                <ChevronRight size={24} stroke='#C66060' className={'max-md:hidden'} />
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className='flex flex-col w-full'>
                    ...
                    {stage === 'otp' ? (
                        <div className='flex items-center justify-center gap-6 w-fit mx-auto'>
                            <button type='button' onMouseDown={() => setStage(prev => ['username', 'email', 'game', 'otp'][['username', 'email', 'game', 'otp'].indexOf(prev) - 1] as 'username' | 'email' | 'otp' | 'game')} className={cn('text-white mx-auto bg-[#B8B1B1] disabled:opacity-60 w-40 md:w-44 h-14 rounded-full mt-12', stage !== 'otp' && 'hidden')}>Back</button>
                        ...
                        <div className='flex items-center justify-center gap-6 w-fit mx-auto'>
                            <button type='button' disabled={stage === 'username'} onMouseDown={() => setStage(prev => ['username', 'email', 'game', 'otp'][['username', 'email', 'game', 'otp'].indexOf(prev) - 1] as 'username' | 'email' | 'otp' | 'game')} className={cn('text-white mx-auto bg-[#B8B1B1] disabled:opacity-60 w-40 md:w-44 h-14 rounded-full mt-12', stage === 'username' && 'hidden')}>Back</button>
                            <button 
